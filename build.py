...
/* Click swaps the still for the Vimeo player. Nothing third-party loads until
   ...
   TOUCH: that trick does not survive. A tap on the PAGE does not carry into a
   cross-origin iframe on iOS, so the autoplay request fails outright and the
   visitor is left with a paused, muted player needing two more taps. So on
   touch we do NOT request autoplay at all: without it Vimeo never enters its
   muted-retry path, and the player appears with its own play control. We still
   attempt play(), which Android often allows; where it is refused, the next tap
   lands directly on Vimeo's button, which IS a gesture on the video element and
   therefore starts it WITH SOUND. Two taps, never a mute to undo. */
      ...
      try {
        ...
        p.ready().then(function () {
          return p.setMuted(false).then(function () { return p.setVolume(1); })
            ...
            ['catch'](function () {
              /* Refused. On touch that is expected and fine: the player is
                 sitting there unmuted with its own play button, so their next
                 tap starts it with sound. On desktop, turning the sound on can
                 itself get playback rejected, and leaving someone who just
                 pressed play looking at a frozen frame is worse than a silent
                 reel, so fall back to muted and keep moving. */
              if (touch) return;
