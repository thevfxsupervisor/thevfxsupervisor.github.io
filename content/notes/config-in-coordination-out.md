...
![A multi-agent fleet: a pull task keeps every clone current, one shared config directory feeds every seat on a box, and each seat also has its own clone, cursor and hook. On the right, the seats coordinate through four separate channel surfaces.](/static/fleet-architecture.html)
